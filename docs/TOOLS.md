...
- 
 ...
 - **Filesystem**: `read_file`, `write_file`, `list_files`, `search_files`, `delete_file`.
 ...
 - **Media & Analytics**: `analyze_media` (natively processes images, audio, video, and PDFs).
 - **Generators**: `image_generator`, `audio_generator`, `video_generator`.
 - **System**: `execute_shell` (bound by security configurations), `system_status` (health metrics and network latency).
- - **Delegation**: `supervised_delegate` (DAG task orchestration), `invoke_agent` (single specialist delegation).
